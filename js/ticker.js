...
if ( windowWidth >= 768 ) { 
  ...
  $('.typed-first').typeIt({
    ...
    .tiSettings({speed: 48}).tiType("<h3>Thriving Families</h3>").tiPause(450).tiDelete()
    .tiSettings({speed: 44}).tiType("<h3>Safer Communities</h3>").tiPause(400).tiDelete()
    .tiSettings({speed: 40}).tiType("<h3>Better Schools</h3>").tiPause(350).tiDelete()
    .tiSettings({speed: 36}).tiType("<h3>Growing Economies</h3>").tiPause(300).tiDelete()
    .tiSettings({speed: 32}).tiType("<h3>Engaged Citizens</h3>").tiPause(250).tiDelete()
    .tiSettings({speed: 28}).tiType("<h3>Responsive Policies</h3>").tiPause(200).tiDelete()
    .tiSettings({speed: 24}).tiType("<h3>Responsible Governance</h3>").tiPause(175).tiDelete()
    .tiSettings({speed: 20}).tiType("<h3>Vibrant Cultures</h3>").tiPause(150).tiDelete()
    .tiSettings({speed: 18}).tiType("<h3>Rigorous Science</h3>").tiPause(125).tiDelete()
    .tiSettings({speed: 16}).tiType("<h3>Innovative Business</h3>").tiPause(100).tiDelete()
    .tiSettings({speed: 14}).tiType("<h3>Pressing Challenges</h3>").tiPause(90).tiDelete()
    .tiSettings({speed: 12}).tiType("<h3>Emerging Opportunities</h3>").tiPause(80).tiDelete()
    .tiSettings({speed: 10}).tiType("<h3>Human Development</h3>").tiPause(70).tiDelete()
    .tiSettings({speed: 9}).tiType("<h3>Social Discourse</h3>").tiPause(60).tiDelete()
    .tiSettings({speed: 8}).tiType("<h3>Creative Collaboration</h3>").tiPause(50).tiDelete()
    .tiSettings({speed: 7}).tiType("<h3>Unbiased Analysis</h3>").tiPause(40).tiDelete()
    .tiSettings({speed: 6}).tiType("<h3>Data Literacy</h3>").tiPause(30).tiDelete()
    .tiSettings({speed: 5}).tiType("<h3>Effective Planning</h3>").tiPause(20).tiDelete()
    .tiSettings({speed: 4}).tiType("<h3>Actionable Evaluations</h3>").tiPause(10).tiDelete()
    .tiSettings({speed: 3}).tiType("<h3>Healthy Aging</h3>").tiPause(0).tiDelete()
    .tiSettings({speed: 2}).tiType("<h3>Lifelong Learning</h3>").tiPause(0).tiDelete()
    .tiSettings({speed: 1}).tiType("<h3>Productive Conversation</h3>").tiPause(0).tiDelete()
